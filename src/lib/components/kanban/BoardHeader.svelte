...
<div class="border-b border-gray-200/50 bg-gradient-to-r from-white/95 via-gray-50/95 to-white/95 px-8 py-6 backdrop-blur-xl dark:border-gray-700/50 dark:from-gray-900/95 dark:via-gray-800/95 dark:to-gray-900/95 shadow-lg">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-6">
			...
			{:else}
				<button
					class="group relative text-left text-3xl font-black text-gray-900 transition-all duration-300 hover:text-transparent hover:bg-gradient-to-r hover:from-blue-600 hover:to-purple-600 hover:bg-clip-text dark:text-white"
					onclick={() => {
						editingTitle = true;
						titleInput = board.title;
					}}
				...
				>
					{board.title}
					<div class="absolute -bottom-1 left-0 h-1 w-0 bg-gradient-to-r from-blue-600 to-purple-600 transition-all duration-300 group-hover:w-full"></div>
				</button>
			{/if}
		...
		<div class="flex items-center gap-6">
			...
			<!-- View mode buttons -->
			<div class="flex items-center gap-1 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 p-1.5 shadow-inner dark:from-gray-700 dark:to-gray-800">
				<button
					onclick={() => kanbanStore.setViewMode('board')}
					class="relative rounded-lg px-4 py-2 text-sm font-semibold transition-all duration-200 {kanbanStore.state.viewMode === 'board'
				...
				<button
					...
					class="relative rounded-lg px-4 py-2 text-sm font-semibold transition-all duration-200 {kanbanStore.state.viewMode === 'list'
						? 'bg-gradient-to-br from-white to-gray-50 text-blue-600 shadow-lg dark:from-gray-600 dark:to-gray-700 dark:text-blue-400'
						: 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100'} opacity-50 cursor-not-allowed"
					disabled
				>
					<svg class="inline-block w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<button
					...
					class="relative rounded-lg px-4 py-2 text-sm font-semibold transition-all duration-200 {kanbanStore.state.viewMode === 'calendar'
						? 'bg-gradient-to-br from-white to-gray-50 text-blue-600 shadow-lg dark:from-gray-600 dark:to-gray-700 dark:text-blue-400'
						: 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100'} opacity-50 cursor-not-allowed"
					disabled
				>
			...
			<div class="relative">
				...
				>
					<svg class="h-5 w-5 text-gray-600 transition-transform duration-200 group-hover:rotate-90 dark:text-gray-300" fill="currentColor" viewBox="0 0 20 20">
						<path
				...
				{#if showMenu}
					<div
						class="absolute right-0 z-20 mt-3 w-56 overflow-hidden rounded-xl border border-gray-200/50 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-gray-700/50 dark:bg-gray-800/95"

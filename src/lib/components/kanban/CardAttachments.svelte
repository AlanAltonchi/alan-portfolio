<script lang="ts">
	...
	import { Upload, X, Download, FileText, Image, File } from 'lucide-svelte';
	...
	async function loadAttachments() {
		...
		if (!error && attachmentsData) {
			...
			const userIds = [...new Set(attachmentsData.map(a => a.uploaded_by).filter((id): id is string => id !== null))];
...
<div class="space-y-4">
	...
	{:else}
		...
		{#if attachments.length > 0}
			<div class="space-y-2">
				{#each attachments as attachment}
					...
					<div class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800 rounded-md group">
						<Icon class="w-8 h-8 text-gray-400 dark:text-gray-500 flex-shrink-0" />
						...
						<div class="flex-1 min-w-0">
							<div class="flex items-center gap-2">
								<a
									...
									class="text-sm font-medium text-blue-600 dark:text-blue-400 
										hover:text-blue-700 dark:hover:text-blue-300 truncate"
								>
							...
							<div class="text-xs text-gray-500 dark:text-gray-400 mt-1">
								Uploaded by {attachment.uploader?.full_name || attachment.uploader?.email || 'Unknown'}
		...
		<button
			...
			class="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium 
				text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 

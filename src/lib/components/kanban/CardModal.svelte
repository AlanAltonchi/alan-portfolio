...
{#if isOpen}
	...
	>
		<!-- Modal Content -->
		<div 
			class="relative w-full max-w-4xl h-[90dvh] flex flex-col rounded-2xl bg-gradient-to-br from-gray-50/95 to-white/95 backdrop-blur-xl shadow-2xl dark:from-gray-900/95 dark:to-gray-800/95"
		...
		>
			...
			<button
				onclick={onClose}
				class="absolute cursor-pointer right-4 top-4 z-10 rounded-full bg-white/80 p-2 text-gray-500 transition-all hover:bg-white hover:text-gray-700 dark:bg-gray-800/80 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-200"
			...
			<!-- Header -->
			<div
				class="relative overflow-hidden border-b border-gray-200/20 bg-gradient-to-r from-blue-500/5 via-purple-500/5 to-pink-500/5 px-8 pb-6 pt-8 backdrop-blur-sm dark:border-gray-700/20 dark:from-blue-500/10 dark:via-purple-500/10 dark:to-pink-500/10"
			>
				<div class="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 blur-3xl"></div>
				<div class="absolute -left-20 -bottom-20 h-40 w-40 rounded-full bg-gradient-to-br from-purple-500/20 to-pink-500/20 blur-3xl"></div>
				...
				<div class="relative flex-1 pr-12">
					{#if isEditing}
						...
						/>
					{:else}
						<h2 class="bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-2xl font-bold text-transparent dark:from-white dark:to-gray-300">
							{title}
						</h2>
					{/if}

					<div class="mt-3 flex items-center gap-4">
						<span class="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-blue-500/10 to-purple-500/10 px-3 py-1 text-sm font-medium text-gray-700 backdrop-blur-sm dark:from-blue-500/20 dark:to-purple-500/20 dark:text-gray-300">
							<div class="h-2 w-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-500"></div>
							{card.column?.title || 'Unknown Column'}
						</span>
						{#if dueDate}
							<span class="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-orange-500/10 to-red-500/10 px-3 py-1 text-sm font-medium text-gray-700 backdrop-blur-sm dark:from-orange-500/20 dark:to-red-500/20 dark:text-gray-300">
			...
			<!-- Tabs -->
			<div class="flex gap-1 border-b border-gray-200/20 bg-gray-50/50 px-8 backdrop-blur-sm dark:border-gray-700/20 dark:bg-gray-800/50">
				<button
					onclick={() => (activeTab = 'details')}
					class="group relative rounded-t-xl px-4 py-3 text-sm font-medium transition-all duration-200 {activeTab === 'details'
				...
				>
					...
					{#if activeTab === 'details'}
						<div class="absolute inset-x-0 -bottom-px h-px bg-gradient-to-r from-transparent via-white to-transparent"></div>
					{/if}
				</button>
				...
				>
					...
					{#if checklistCount > 0}
						<span class="rounded-full {activeTab === 'checklists' ? 'bg-white/20 text-white' : 'bg-gradient-to-r from-blue-500/10 to-purple-500/10 text-gray-700 dark:text-gray-300'} px-2 py-0.5 text-xs font-semibold">
							{checklistCount}
						</span>
					{/if}
				...
				>
					...
					{#if attachmentCount > 0}
						<span class="rounded-full {activeTab === 'attachments' ? 'bg-white/20 text-white' : 'bg-gradient-to-r from-blue-500/10 to-purple-500/10 text-gray-700 dark:text-gray-300'} px-2 py-0.5 text-xs font-semibold">
							{attachmentCount}
						</span>
					{/if}
				...
				>
					...
					{#if commentCount > 0}
						<span class="rounded-full {activeTab === 'comments' ? 'bg-white/20 text-white' : 'bg-gradient-to-r from-blue-500/10 to-purple-500/10 text-gray-700 dark:text-gray-300'} px-2 py-0.5 text-xs font-semibold">
							{commentCount}
						</span>
					{/if}
				...
				{#if activeTab === 'details'}
					<div class="space-y-6 h-full">
						...
						<div class="grid grid-cols-2 gap-4">
							<div class="rounded-xl bg-gradient-to-br from-gray-50/50 to-gray-100/50 p-4 backdrop-blur-sm dark:from-gray-800/50 dark:to-gray-900/50">
								<h3
							...
							<div class="rounded-xl bg-gradient-to-br from-gray-50/50 to-gray-100/50 p-4 backdrop-blur-sm dark:from-gray-800/50 dark:to-gray-900/50">
								...
								<select
									...
									class="w-full rounded-lg border border-gray-200/50 bg-white/70 px-3 py-2
										text-gray-900 backdrop-blur-sm transition-all duration-200 hover:bg-white/80 focus:border-blue-500/50 focus:outline-none focus:ring-2 focus:ring-blue-500/20 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-700/50
						...
						<!-- Description -->
						<div class="rounded-xl bg-gradient-to-br from-gray-50/50 to-gray-100/50 p-4 backdrop-blur-sm dark:from-gray-800/50 dark:to-gray-900/50">
							<h3 class="mb-3 text-sm font-semibold text-gray-700 dark:text-gray-300">Description</h3>
							{#if isEditing}
								<CardEditor bind:content={description} />
							{:else if description}
								<div class="prose dark:prose-invert max-w-none rounded-lg bg-white/50 p-4 backdrop-blur-sm dark:bg-gray-800/50">
									{@html renderMarkdown(description)}
								</div>
							{:else}
								<p class="rounded-lg bg-white/50 p-4 text-gray-500 italic backdrop-blur-sm dark:bg-gray-800/50 dark:text-gray-400">No description provided</p>
							{/if}
						</div>
					</div>
